...
const DepartmentCoursesView = ({ onCourseClick }) => {
	...
	/**
	 * Extract unique years from the course data for the dropdown
	 */
	const academicYears = useMemo(() => {
		const unique = [...new Set(courses.map((c) => c.year))].filter(Boolean);
	...
	const filteredCourses = useMemo(() => {
		...
		return courses.filter((course) => {
			...
			return matchesSearch && matchesYear && matchesArchiveStatus;
		});
	}, [courses, filters.searchQuery, selectedYear, showArchived]);
	...
	const getDisplayTitle = () => {
		const yearPrefix =
			selectedYear === "All" ? "All" : `${selectedYear} Year`;
				...
				{filteredCourses.length > 0 ? (
					...
					<div className="bg-white dark:bg-gray-800 border border-dashed border-gray-200 dark:border-gray-700 rounded-3xl p-12 text-center">
						<p className="text-sm text-gray-400 italic">
							...
							matching your search or year filter.
						</p>
					</div>
				)}
			</main>
