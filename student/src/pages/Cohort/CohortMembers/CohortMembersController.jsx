...
const CreateGroupModal = ({
	...
	const handleSubmit = async (e) => {
		...
		const maxSelectable = isProfessor
			? maxGroupMembers
		...
		const currentGroupSize = isProfessor
			...
			<div className="bg-white dark:bg-gray-800 rounded-xl w-full max-w-2xl max-h-[80vh] overflow-hidden flex flex-col">
				...
				<div className="flex-1 overflow-y-auto p-4">
					<form onSubmit={handleSubmit} className="space-y-3">
						...
						{/* Info about capacity including creator and min/max */}
				...
				const csvData = individualMembers
					.map((member) => {
						...
						const assignmentScores = assignments.map(
							(assignment) => {
								const gradeKey = `${member.id}_${assignment.id}`;
