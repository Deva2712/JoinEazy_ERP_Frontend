...
export const apiCall = async (endpoint, options = {}) => {
	...
	if (USE_MOCK_API) {
		...
		if (endpoint.match(/^\/cohort\/(\d+)\/details$/)) {
			...
			const sharedCohortData = Mocks.SHARED_COHORTS.find(
				(c) => c.id === courseId,
			);
			...
			if (courseDetails || sharedCohortData) {
				...
				const memberCount =
					sharedCohortData?.member_count ||
					courseDetails?.memberCount ||
					45;
				const groupCount =
					sharedCohortData?.group_count ||
					courseDetails?.groupCount ||
					0;
						...
						detail_sections: courseDetails?.detail_sections || [
							{
								...
								subsec_description:
									sharedCohortData?.cohort_description ||
		...
		if (endpoint.match(/^\/job-tray$/)) {
			...
			// 3. LEAVE APPLICATIONS: Filter Action Required Apps and Pending Requests/Substitutions
			...
			const leaveApps = leaveData.applications || [];

			// Process personal leave applications that require action (Rejections)
			leaveApps.forEach((app) => {
				if (app.status === "Rejected" && !app.isArchived) {
					...
					if (app.leaveApproval?.Dean?.status === "Rejected")
						remarks.push(`Dean: ${app.leaveApproval.Dean.remark}`);
					if (app.leaveApproval?.HR?.status === "Rejected")
						remarks.push(`HR: ${app.leaveApproval.HR.remark}`);

					jobs.push({
						id: `leave-app-${app.id}`,
						title: `Your leave application for ${app.leaveType} requires action.`,
			...
			// Handle Role-Specific Pending Tasks
			if (role === "hod") {
				// HOD: Process incoming leave requests from faculty
				const incomingReqs = leaveData.incomingLeaveRequests || [];
				incomingReqs.forEach((req) => {
					if (
						...
						jobs.push({
							id: `incoming-leave-${req.id}`,
							title: `Leave approval requested by ${req.professorName}.`,
							type: "LEAVE_APPROVAL",
							status: "Action Required",
							link: "/leave-applications/manage-requests",
							statusNote: `${req.leaveType}: ${formatDate(req.fromDate)} to ${formatDate(req.toDate)}`,
							createdAt: req.appliedAt || nowISO,
						});
					}
				});
			} else {
				// PROFESSOR: Process incoming substitution requests
				const subRequests = leaveData.substitutionRequests || [];
				subRequests.forEach((sub) => {
					if (sub.status === "Pending") {
						...
						jobs.push({
							...
							createdAt: sub.requestedAt || nowISO,
						});
					}
				});
			}
		...
		if (endpoint.match(/^\/notifications$/)) {
			...
			const scheduledMeetings = Mocks.MOCK_SCHEDULED_MEETINGS || [];
			...
			if (
				...
				try {
					...
					const incomingRequest =
						Mocks.MOCK_MEETING_REQUESTS.incoming.find(
							(r) =>
								r.id === requestId ||
					...
					if (scheduledMeeting && action === "reschedule") {
						try {
							...
							scheduledMeeting.rescheduleReason =
								body.rescheduleReason || null;

							if (body.mode) {
								scheduledMeeting.type =
									body.mode.charAt(0).toUpperCase() +
									body.mode.slice(1);
							}
							...
							return {
								...
								data: {
									message:
										"Meeting updated and rescheduled successfully",
								},
							};
						} catch (err) {
							return {
								success: false,
								message: "Failed to parse reschedule data",
							};
						}
					}
					...
					if (incomingRequest) {
						if (action === "accept") {
							...
							const newScheduled = {
								...
								participantName:
									incomingRequest.participantName,
								participantId: incomingRequest.participantId,
								participantRole:
									incomingRequest.participantRole,
								...
								location:
									incomingRequest.type === "Online"
										? "Virtual Meeting"
										: body.location || "Department Office",
								meetingLink:
									incomingRequest.type === "Online"
										? body.meetingLink ||
		...
		if (
			...
			try {
				...
				if (status === "final") {
					...
					if (!courseData.finalizedSections)
						courseData.finalizedSections = {};
					if (!courseData.finalizedSections[date])
						courseData.finalizedSections[date] = [];

					if (
						!courseData.finalizedSections[date].includes(
							targetSection,
						)
					) {
					...
					if (!courseData.drafts[targetSection])
						courseData.drafts[targetSection] = {};
		...
		// POST: Record academic experience for a professor
		if (
			endpoint.match(/^\/professor\/experience$/) &&
			options.method === "POST"
		) {
			try {
				if (!options.body) throw new Error("Missing request body");
				...
				const bodyData = JSON.parse(options.body);
				const { leaveId, conferenceName, experience, facultyId } =
					bodyData;

				if (!leaveId || !conferenceName || !facultyId) {
					throw new Error(
						"Missing required fields: leaveId, conferenceName, or facultyId",
					);
				}

				// 1. Find the leave application to extract the date
				const allApplications = [
					...Mocks.MOCK_PROFESSOR_LEAVE_DATA.applications,
					...Mocks.MOCK_HOD_LEAVE_DATA.applications,
				];
				...
				// 2. Format the date from the application to yyyy-mm-dd
				const applicationDate = new Date(application.fromDate)
					.toISOString()
					.split("T")[0];

				// 3. Update Faculty History
				const facultyMember =
					Mocks.MOCK_DEPARTMENT_FACULTY_DATA.faculty.find(
						(f) => f.id === facultyId,
					);

				if (!facultyMember) throw new Error("Faculty member not found");

				if (!facultyMember.conferences) {
					facultyMember.conferences = [];
				}

				const newEntry = {
					conferenceName,
					experience: experience || "",
					date: applicationDate, // Extracted from leave application
				};

				facultyMember.conferences.push(newEntry);

				return {
					status: "success",
					message:
						"Academic experience recorded with application date",
					data: newEntry,
				};
			} catch (err) {
				return {
			...
			if (endpoint.match(/^\/sessions\/documents\/.+/)) {
				...
				if (method === "POST") {
					...
					uploadedKeys.forEach((docType) => {
						const courseDocs =
							Mocks.MOCK_COURSE_DOCUMENTS[courseId];
						const existingIndex = courseDocs.findIndex(
							(d) => d.type === docType,
						);
						...
						const newDoc = {
							...
							version:
								existingIndex !== -1
									? courseDocs[existingIndex].version + 1
									: 1,
		...
		// --- DEPARTMENT: OVERVIEW ---
		if (
			endpoint.match(/^\/department\/overview\/?$/) &&
			options.method === "GET"
		) {
			if (currentRole !== "hod")
				return {
		...
		// --- DEPARTMENT: ALERTS ---
		if (
			endpoint.match(/^\/department\/alerts\/?$/) &&
			options.method === "GET"
		) {
			if (currentRole !== "hod") {
				return {
			...
			Mocks.SHARED_COHORTS.forEach((cohort) => {
				const attendance = Mocks.MOCK_ATTENDANCE_DATA[cohort.id];
				const scheduleData = Mocks.MOCK_COURSE_SCHEDULE.find(
					(s) => s.id === cohort.id,
				);

				if (!attendance || !scheduleData) return;

				cohort.sections.forEach((section) => {
					const sectionLogs =
						attendance.logs[section.section_name] || {};
					const loggedDates = Object.keys(sectionLogs).map((d) =>
						new Date(d).toDateString(),
					);

					const scheduledDays = scheduleData.schedule
						.filter(
							(s) =>
								s.batchSection === section.section_name ||
								s.batchSection.includes(section.section_name),
						)
						.map((s) => s.day);

					const lastTwoExpectedDates = [];

					for (
						let i = 1;
						i <= 14 && lastTwoExpectedDates.length < 2;
						i++
					) {
						let pastDate = new Date(today);
						pastDate.setDate(today.getDate() - i);

						const dayName = pastDate.toLocaleDateString("en-US", {
							weekday: "long",
						});
						if (scheduledDays.includes(dayName)) {
							lastTwoExpectedDates.push(pastDate);
						}
					}

					const missingDates = lastTwoExpectedDates.filter(
						(expectedDate) =>
							!loggedDates.includes(expectedDate.toDateString()),
					);

					if (missingDates.length >= 2) {
						alerts.push({
							id: `alert-attn-${cohort.id}-${section.section_name}`,
							category: "Attendance",
							priority: "high",
							title: "Attendance Overdue",
							message: `Attendance not marked for ${section.section_name} (${section.course_code}) for the last two classes.`,
							metadata: {
								course: cohort.cohort_name,
								section: section.section_name,
								professor: section.professor || "Unassigned",
			...
			const facultyList = Mocks.MOCK_DEPARTMENT_FACULTY_DATA.faculty;
			const feedbackList = Mocks.MOCK_DEPARTMENT_FACULTY_DATA.feedback;

			facultyList.forEach((fac) => {
				const facFeedback = feedbackList.filter(
					(f) => f.facultyId === fac.id,
				);

				if (facFeedback.length > 0) {
					const avgRating =
						facFeedback.reduce(
					...
					if (avgRating < 2.5) {
						alerts.push({
							id: `alert-rating-${fac.id}`,
							category: "Performance",
							priority: "medium",
							title: "Low Faculty Rating",
							message: `${fac.name} has a current average rating of ${avgRating.toFixed(1)}, which is below the threshold.`,
							metadata: {
								facultyId: fac.id,
								currentRating: avgRating,
								totalFeedbacks: facFeedback.length,
							},
						});
					}
				}
		...
		if (
			endpoint.match(/^\/department\/courses\/?$/) &&
			options.method === "GET"
		) {
			return {
				success: true,
				data: {
					courses: [
						...Mocks.SHARED_COHORTS,
						...Mocks.MOCK_ARCHIVED_COURSES,
					],
		...
		if (
			endpoint.match(/^\/department\/courses\/document-status\/?$/) &&
			options.method === "POST"
		) {
			if (currentRole !== "hod")
				return {
			...
			const { courseId, type, status, comments } = JSON.parse(
				options.body,
			);

			if (Mocks.MOCK_COURSE_DOCUMENTS[courseId]) {
				const docIndex = Mocks.MOCK_COURSE_DOCUMENTS[
					courseId
				].findIndex((d) => d.type === type);

				if (docIndex !== -1) {
					Mocks.MOCK_COURSE_DOCUMENTS[courseId][docIndex].status =
						status;
					Mocks.MOCK_COURSE_DOCUMENTS[courseId][
						docIndex
		...
		// --- DEPARTMENT: FACULTY ---
		if (
			endpoint.match(/^\/department\/faculty\/?$/) &&
			options.method === "GET"
		) {
			const faculty = Mocks.MOCK_DEPARTMENT_FACULTY_DATA.faculty.map(
				(f) => ({
					...f,
					research: {
						projects: Mocks.MOCK_RESEARCH_PROJECTS.filter(
							(p) =>
								p.professorName === f.name ||
								p.collaborators?.includes(f.name),
						),
						publications: Mocks.MOCK_PUBLICATIONS.filter(
							(pub) =>
								pub.professorName === f.name ||
								pub.coAuthors?.includes(f.name),
						),
					},
				}),
			);
		...
		// --- DEPARTMENT: STUDENTS ---
		if (
			endpoint.match(/^\/department\/students\/?$/) &&
			options.method === "GET"
		) {
			const students = Mocks.MOCK_MENTORING_DATA.map((s) => ({
				...s,
				research: {
					projects: Mocks.MOCK_RESEARCH_PROJECTS.filter(
						(p) =>
							p.professorName === s.name ||
							p.collaborators?.includes(s.name),
					),
					publications: Mocks.MOCK_PUBLICATIONS.filter(
						(pub) =>
							pub.professorName === s.name ||
							pub.coAuthors?.includes(s.name),
					),
				},
				placement:
					Mocks.MOCK_STUDENT_PLACEMENT_DATA.find(
		...
		if (
			endpoint.match(/^\/department\/placements\/?$/) &&
			options.method === "GET"
		) {
			return {
				success: true,
				data: {
					placements: Mocks.MOCK_STUDENT_PLACEMENT_DATA,
					companies: Mocks.MOCK_PLACEMENT_COMPANIES_DATA,
					stats: Mocks.MOCK_DEPARTMENT_PLACEMENT_STATS,
				},
			};
		}

		// --- DEPARTMENT: RESEARCH & FUNDING ---
		if (endpoint.match(/^\/department\/research/)) {
			// GET: Fetch Research Projects, Publications, and Funding Data
			if (
				endpoint.match(/^\/department\/research\/?$/) &&
				options.method === "GET"
			) {
				const departmentFilter = "Computer Science";

				let filteredProjects = Mocks.MOCK_RESEARCH_PROJECTS;
				let filteredPublications = Mocks.MOCK_PUBLICATIONS;
				let filteredGrants = Mocks.MOCK_RESEARCH_GRANT_DATA.requests;

				if (departmentFilter) {
					filteredProjects = Mocks.MOCK_RESEARCH_PROJECTS.filter(
						(project) => project.department === departmentFilter,
					);
					filteredPublications = Mocks.MOCK_PUBLICATIONS.filter(
						(pub) => pub.department === departmentFilter,
					);
					filteredGrants =
						Mocks.MOCK_RESEARCH_GRANT_DATA.requests.filter(
							(grant) => grant.department === departmentFilter,
						);
				}
				...
				return {
					...
					data: {
						...
						publications: filteredPublications.map((p) => ({
							id: p.id,
							title: p.title,
							professorName: p.professorName,
						...
						grantRequests: filteredGrants,
						funding: Mocks.MOCK_DEPARTMENT_FUNDING_DATA,
					},
				};
			}

			// POST: Update Grant Request Status
			if (
				endpoint.match(/^\/department\/research\/grant-status\/?$/) &&
				options.method === "POST"
			) {
				const { requestId, status, adminComments } = JSON.parse(
					options.body,
				);
				...
				if (requestIndex !== -1) {
					Mocks.MOCK_RESEARCH_GRANT_DATA.requests[requestIndex] = {
						...Mocks.MOCK_RESEARCH_GRANT_DATA.requests[
							requestIndex
						],
						status: status,
						adminComments: adminComments,
						lastAdminAction: new Date().toISOString().split("T")[0],
					};

					return {
						success: true,
						message: `Grant request ${requestId} updated to ${status}.`,
						data: Mocks.MOCK_RESEARCH_GRANT_DATA.requests[
							requestIndex
						],
					};
				}
