...
import { Clock, CheckSquare, Square, Calendar, History } from "lucide-react";
import TaskHistoryModal from "./TaskHistoryModal";

/**
 * Component for displaying the list of upcoming tasks.
 * Manages task visibility, sorting by time, and toggling completion.
 */
const UpcomingTasks = ({ tasks: tasksProp = [], onToggleTask }) => {
	...
	useEffect(() => {
		// Only display tasks that haven't been completed
		const activeTasks = tasksProp.filter((t) => !t.completed);
		...
		setVisibleIds((prevVisible) => {
			const currentValid = prevVisible.filter((id) =>
				sortedTasks.some((t) => t.id === id),
			);

			const additional = sortedTasks
				.filter((t) => !currentValid.includes(t.id))
	...
	/**
	 * Triggers the removal animation and notifies the controller to update history.
	 */
		...
		setTimeout(() => {
			...
			setVisibleIds((vids) => {
				...
				const nextTask = allTasks.find(
					(t) => !currentVisible.includes(t.id) && t.id !== taskId,
				);
				return nextTask
	...
	const groupedHistory = historyData.reduce((acc, task) => {
		const date = task.historyDate;
	...
	return (
		<div className="mb-8 bg-white dark:bg-[#1a1d26] rounded-2xl p-4 sm:p-6 border border-gray-200 dark:border-gray-800 shadow-sm">
			{/* Main Header */}
			<div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4 sm:mb-6">
				<div className="flex items-center gap-4">
					...
					<div>
						<h3 className="text-lg sm:text-xl font-bold text-gray-900 dark:text-gray-200 tracking-tight">
							Upcoming Tasks
						</h3>
				...
				<div className="flex items-center justify-between sm:justify-end gap-2">
					...
					<button
						...
						className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-gray-100 dark:bg-white/5 border border-gray-200 dark:border-white/10 hover:bg-gray-200 dark:hover:bg-white/10 transition-colors"
					>
						...
						>
							...
							>
								{removing === task.id ? (
									...
									<Square className="w-5 h-5 text-gray-400 group-hover/item:text-blue-500 transition-colors" />
								)}
							</button>
